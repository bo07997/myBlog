...
很简单，`copy`是浅层次的`copy`，如果你的`copy`对象时比较复杂的对象，例如`map`里面嵌套着`map`，那么它只会单独copy第一层，深层次的对象会和原来的copy对象共享。
...
实际问题:由于线上公用一个优先队列（`python`有实现），每个单独的用户环境要处理这一个队列的时候都要单独的去`copy`这个队列，从上面知道，我们不能用`copy`，因为该队列
